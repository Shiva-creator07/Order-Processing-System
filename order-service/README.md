...
This project was generated as a complete, ready-to-run scaffold — full source, Docker Compose,
and seed data. It has **not** been compiled/run in this environment (no network access to Maven
Central here), so before you screenshot it working or record a demo for your portfolio: run
`docker compose up --build` locally, watch the logs, and fix anything Maven's build output flags
(most likely candidates: a typo in a binding name in `application.yml`, or a Lombok annotation
processing hiccup — both are quick fixes). Once it's running, genuinely understand the saga flow
before you put it on your CV — this is meant to be a real project you can defend in an interview,
not a black box.
